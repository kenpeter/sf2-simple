...
class OnlineExperienceBuffer:
    ...
    def add_experience(self, image: Image.Image, game_state: Dict, action: int, 
                      reward: float, next_image: Image.Image, next_state: Dict, done: bool):
...
Return action number (0-43):"""
    ...
    def perform_online_update(self, batch_size: int = 1):  # Reduced batch size for memory
            ...
            try:
                ...
                if batch[0]["pixel_values"] is not None:
                    pixel_values = torch.stack([
                        ...
                        for item in batch if item["pixel_values"] is not None
                    ])
                ...
                if batch[0]["image_grid_thw"] is not None:
                    image_grid_thw = torch.stack([
                        ...
                        for item in batch if item["image_grid_thw"] is not None
                    ])
...
class OnlineStreetFighterEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.observation_space = spaces.Box(low=0, high=255, shape=(224, 320, 3), dtype=np.uint8)
...
def run_online_training(
    ...
    class LightweightAgent:
        ...
        def get_action(self, obs, info, verbose=False):
            ...
            if distance < 60:
                ...
                action = 21 if self.frame_counter % 20 < 10 else 6  # Kick or move forward
            else:
    ...
    for episode in range(episodes):
        ...
        while step < max_steps:
            ...
            if total_steps % update_frequency == 0 and len(trainer.experience_buffer) >= 1:
